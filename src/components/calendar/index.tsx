...
interface CalendarProps {
  ...
  dateOnChange?: (date: { month: number; year: number; day: number }) => void
}
...
const Calendar = ({
  ...
  const gridViewCpn: {
    [key: string]: { control: React.ReactNode; view: React.ReactNode }
  } = {
    day: {
      control: (
        <ControlDayView
  ...
  return (
    <div className='bg-white rounded-[4px] shadow h-fit'>
      {gridViewCpn[viewMode].control}
      <div className=''>{gridViewCpn[viewMode].view}</div>
    </div>
  )
}
