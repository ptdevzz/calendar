...
const EventItem = ({ type }: EventItemProps) => {
    ...
    return <li className={`mb-1 p-1 pl-6 py-6 rounded-lg relative before:absolute before:w-2  before:h-full before:top-0 before:left-0 overflow-hidden ${classByType[type].wrapper} ${classByType[type].beforeWrapper}`}>
        ...
        <div className="flex items-center gap-x-2 mt-4">
            <img src="https://img.freepik.com/free-vector/smiling-young-man-illustration_1308-174669.jpg?semt=ais_hybrid" className="w-10 h-10 rounded-full" />
            <span className={`text-md underline cursor-pointer ${classByType[type].link}`}>View Client Profile</span>
        </div>
    </li>
}
